  ...
  const sortedRestaurants = useMemo(() => {
    ...
    if (bestMode) {
      ...
      return restaurantsWithScores
        .map((r) => ({
          ...r,
          normalized_wilson_score: (r.wilson_score / maxScore) * 100,
        }))
        .sort((a, b) => b.wilson_score - a.wilson_score)
        .slice(0, 25);
    } else {
      const scores = restaurantsWithScores.map((r) => r.wilson_score);
      ...
      return restaurantsWithScores
        .map((r) => ({
          ...
          normalized_wilson_score:
            ((r.wilson_score - minS) / (maxS - minS)) * 100,
        }))
  ...
  return (
    <div className="max-w-7xl mx-auto p-4">
      <div className="my-4 space-y-2 bg-gray-200 rounded p-2">
        ...
        <div className="flex items-center gap-2 bg-gray-300 rounded p-2">
          ...
          <label className="inline-flex items-center cursor-pointer">
            ...
            />
            <div className="relative w-11 h-6 bg-blue-600 rounded-full peer after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all after:translate-x-0 peer-checked:after:translate-x-full"></div>
          </label>
      ...
      {!showMap ? (
        ...
        <RestaurantMap restaurants={validRestaurantsForMap} />
      )}
    </div>
  );
}
